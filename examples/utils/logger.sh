#!/usr/bin/env bash
# shellcheck disable=SC2155,SC2163,SC2207,SC2154


# shellcheck disable=SC2065,SC1091
test -z "$(declare -f log)" >/dev/null && source "$(realpath "./")/utils/colors.sh"
...
kbx_end() {
  ...
  if test -n "${kbx_tail_pid:-}"; then
    sleep 0.3
    kill -9 "${kbx_tail_pid}" >/dev/null 2>&1 || true
  fi
...
# logger.sh - Logging utility for shell scripts
